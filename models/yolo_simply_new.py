    ...
    def forward(self, x):
        ...
        # for i in range(self.nl):
            ...
            '''
                ...
                j = 0意味着 anchor 取的数值是[4,5] [23,29],[146,217]
                p3,p4,p5 每个layer只用到了部分feature
                不知道为啥，一个P3 对应3种 size
            '''
            ...
            if not self.training:
                ...
                box_xy = (y[:, :, :, 0:2] * 2. + (- 0.5) + self.grid[i][j:j + 1, :, :, :]) * self.stride[ i]  # xy
                box_wh = (y[:, :, :, 2:4] * 2) * (y[:, :, :, 2:4] * 2) * self.anchor_grid[i][j:j + 1, :, :, :]
                y1 = y[:, :, :, 4:5]
                box_xy = torch.permute(box_xy, [0, 3, 1, 2])
                box_wh = torch.permute(box_wh, [0, 3, 1, 2])
...
class Model(nn.Module):
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None):  # model, input channels, number of classes
        ...
        if isinstance(m, Detect):
            s = 128  # 2x min stride
            print(" torch.tensor([s / x.shape[-2] for x in self.forward(torch.zeros(1, ch, s, s))]) ",torch.tensor([s / x.shape[-2] for x in self.forward(torch.zeros(1, ch, s, s))]))
