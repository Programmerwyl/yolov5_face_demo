...
def img_process(orgimg,image_shape=(512,960),stride_max=32):
    '''
    图像预处理
    '''
    long_side = np.max(image_shape)
    ...
    check_img_size(long_side, s=stride_max)  # check img_size

    img = letterbox(img0, new_shape=image_shape,auto=False)[0] # auto True最小矩形   False固定尺度
    # Convert
...
def letterbox(img, new_shape=(512, 960), color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True):
