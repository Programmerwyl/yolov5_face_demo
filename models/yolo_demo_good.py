...
class Detect(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            y = x[i].sigmoid()
            # y[:, :, :, 0:2] = (y[:, :, :, 0:2] * 2 +(-0.5)+ self.grid[i][j:j+1,:,:,:]) * self.stride[i]  # xy
            # y[:, :, :, 2:4] = (y[:, :, :, 2:4] * 2) *(y[ :, :, :, 2:4] * 2) * self.anchor_grid[i][j:j+1,:,:,:]  # wh
            # y[:, :, :, 2:4] = (y[:, :, :, 2:4] * 2) *(y[ :, :, :, 2:4] * 2)   # wh
            # y[:, :, :, 2:4] = y[:, :, :, 2:4] * self.anchor_grid[i][j:j+1,:,:,:]  # wh
