...
from models.common_simply import Conv, Bottleneck, SPP, DWConv, Focus, BottleneckCSP, C3, ShuffleV2Block, Concat, NMS, autoShape, StemBlock, BlazeBlock, DoubleBlazeBlock
...
class Detect(nn.Module):
        ...
        if self.export_cat:
            for i in range(self.nl):
                ...
                # x[i] = x[i].view(bs, self.na, self.no, ny, nx).permute(0, 1, 3, 4, 2).contiguous()
                    ...
                    # interval = number//3
                    # x[i] = torch.cat([x[i][:,:interval,:,:],x[i][:,interval:2*interval,:,:],x[i][:,2*interval:3*interval,:,:]],dim=0)
                    ...
                    x11 = feature[ :, :, :, 5:15]
                    x22 = feature[:, :, :, 15:15 + self.nc].sigmoid()

                    x11 = torch.permute(x11,[0,3,2,1])
                    x22 = torch.permute(x22, [0, 3, 2, 1])
                    x33 = torch.cat((x11,x22),1)

                    x55 = feature[ :, :, :, 0:5].sigmoid()
                    x55 = torch.permute(x55, [0, 3, 2, 1])

                    x66 = torch.cat((x33,x55),dim=1)
                    x66 = torch.permute(x66,[0,3,2,1])
                    y = y + x66
    ...
    def _make_grid_new(self,nx=20, ny=20,i=0):
        ...
        # xv = xv.reshape(1,1,xv.shape[0],xv.shape[1])
        # yv = yv.reshape(1, 1, yv.shape[0], yv.shape[1])
        # xyv = torch.cat((xv, yv), dim=1)
        # xyv = torch.permute(xyv,(0,2,3,1))
        # grid = xyv.expand((self.na, ny, nx, 2)).float()
